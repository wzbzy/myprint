...
function selectCell(highlightColumn, cellList) {
  ...
  elementUtil.setCurrentElement(cellList);
}
...
function computedTableCell(tableElement, table, tableHeadListList) {
  ...
  for (let row = 0; row < tableHeadListList.length; row++) {
    ...
    for (let col = 0; col < tableHeadListListElement.length; col++) {
      ...
      const tdY = numberUtil.default.sub(tdRect.y, tableRect.y);
      const tdX = numberUtil.default.sub(tdRect.x, tableRect.x);
      ...
      height = numberUtil.default.div(tableCellElement.runtimeOption.height, tableCellElement.rowspan);
    }
  ...
  for (let j = 0; j < tableHeadList.length; j++) {
    ...
    if (childList != null && childList.length > 0) {
      ...
      for (let tableHeadCellElement of childList) {
        ...
        width = numberUtil.default.sub(width, tableHeadCellElement.width);
      }
  ...
  for (let j = 0; j < tableHeadList.length; j++) {
    ...
    if (childList != null && childList.length > 0) {
      ...
      for (let tableHeadCellElement of childList) {
        ...
        colspan += tableHeadCellElement.colspan;
        width = numberUtil.default.sum(width, tableHeadCellElement.width);
        widthEx = numberUtil.default.sum(widthEx, tableHeadCellElement.runtimeOption.width);
      }
      tableHeadCell.colspan = colspan;
      elementUtil.setElementWidthPx(widthEx, tableHeadCell);
    } else {
      tableHeadCell.colspan = 1;
    }
  }
...
function addStatisticsRow(tableElement) {
  ...
  tableElement.tableBodyList[0].map((v) => {
    const element = utils.parse(utils.stringify(v, "parent", "target", "elementList"), vueDemi.reactive({}));
    element.id = void 0;
    element.data = void 0;
    element.option.formatter = void 0;
    element.runtimeOption.cellType = "Statistics";
    elementUtil.installParentElement(tableElement, element);
...
function previewRowStatisticsList(rowList, statisticsList, headList, statisticsDisplayType) {
  ...
  for (let col = 0; col < rowList.length; col++) {
    ...
    const statisticsCell = utils.parse(utils.stringify(head, "parent"), vueDemi.reactive({}));
  ...
  for (let previewData of previewDataList) {
    for (let col of colList) {
      ...
      for (let statisticsWrapper of statisticsWrapperList) {
        ...
        if (numberUtil.default.isNumber(data)) {
  ...
  for (let col of colList) {
    for (let statisticsWrapper of statisticsListWrapper[col]) {
      ...
      if (cell.statisticsType == "Sum" || cell.statisticsType == "Max" || cell.statisticsType == "Min") {
        ...
        cell.data = numberUtil.default.div(statisticsWrapper.value, statisticsWrapper.count);
...
exports.addStatisticsRow = addStatisticsRow;
exports.cellIsContinue = cellIsContinue;
exports.computeColumnColspan = computeColumnColspan;
exports.computedCellRect = computedCellRect;
exports.computedDisableColumn = computedDisableColumn;
exports.computedTableCell = computedTableCell;
exports.findFromLeftCell = findFromLeftCell;
exports.findTableHeadDeep = findTableHeadDeep;
exports.findUpperCell = findUpperCell;
exports.findUpperCellIndex = findUpperCellIndex;
exports.getChildByParent = getChildByParent;
exports.getTableCell = getTableCell;
exports.getTableCellDown = getTableCellDown;
exports.handleTableCellInitHeight = handleTableCellInitHeight;
exports.initTableCell = initTableCell;
exports.lastHeadList = lastHeadList;
exports.previewRowStatisticsList = previewRowStatisticsList;
exports.previewTableStatisticsList = previewTableStatisticsList;
exports.recursionColumnDisable = recursionColumnDisable;
exports.recursionForTableCell = recursionForTableCell;
exports.recursionHandleTableHead = recursionHandleTableHead;
exports.selectCell = selectCell;
exports.statisticsData = statisticsData;
exports.tableHeadList2Nest = tableHeadList2Nest;
exports.tableRealCol = tableRealCol;
