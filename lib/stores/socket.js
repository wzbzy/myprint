...
let lockReconnect;
const useSocket = pinia.defineStore("myPrintSocket", {
  state: () => {
    return {
      socket: void 0,
      timer: void 0,
  ...
  actions: {
    INIT_SOCKET() {
      ...
      const init = () => {
        this.socket.onopen = function(_event) {
          ...
          stateThis.socket.send(JSON.stringify({
            options: { css: utils.printCssStyle() },
            cmd: "text/css"
          }));
        };
      ...
      const createSocket = () => {
        try {
          stateThis.socket = new WebSocket(config.useConfigStore().clientUrl.replace("https", "ws").replace("http", "ws"));
          init();
        } catch (e) {
          reconnect();
        }
