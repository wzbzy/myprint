...
async function autoPage(previewEl, pageList, panel, previewDataList) {
  ...
  async function autoTableRow(previewContext2, previewDataList2, index) {
    ...
    const rowHeightPx = (() => {
      ...
      const heightPx = devicePixelRatio.unit2px(Number.isFinite(height) ? height : 0, panel);
    ...
    const estimateTableBodyHeightPx = () => {
      ...
      const headerHeightPx = devicePixelRatio.unit2px(headerHeight, panel);
    ...
    for (; i < previewDataList2.length + tableStatisticsSize; i++) {
      ...
      if (await isNeedNewPage(devicePixelRatio.unit2px(previewWrapper.y, panel) + contentHeightPx, devicePixelRatio.unit2px(previewContext2.bottom, panel))) {
        previewWrapper.tableBodyList.pop();
        previewDataTmpList.pop();
        dataTable.statisticsData(previewDataTmpList, statisticsListWrapper);
  ...
  async function newPage() {
    ...
    if (previewContext.panel.pageHeader) {
      ...
      if (previewContext.top == null || !(previewContext.top > 0)) {
        previewContext.top = numberUtil.default.sumScale(preview.y ?? 0, preview.height ?? 0);
      }
    }
    if (previewContext.panel.pageFooter) {
